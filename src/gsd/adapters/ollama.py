...
# Config-only meta-keys that control project logic but are NOT valid ChatOllama
# constructor parameters.  Stripped before the kwargs dict is passed to ChatOllama.
_META_KEYS: frozenset[str] = frozenset({"structured_output"})
...
_STRIP_FORMAT_LOGGED: set[str] = set()  # one-time log guard per (model_key, model)
...
def _build_chat_kwargs(
    ...
    MLX policy: when the resolved model config has ``structured_output: false``
    (set for MLX-backed models in models.yaml), the ``format`` kwarg is stripped
    entirely before being passed to ChatOllama.  Ollama's MLX runner does NOT
    support the ``format`` option in ANY form — neither grammar-constrained
    schema dicts nor the simple ``"json"`` mode.  Passing any value causes the
    MLX subprocess to crash at the sampler's first call (``status code: -1``).
    Callers that want JSON output from MLX models must rely on prompt
    engineering; ``_extract_json`` handles prose-wrapped responses robustly.
    ...
    # MLX policy: strip format entirely when the model opts out of structured output.
    # The `structured_output` meta-key lives in models.yaml; it is read here and
    # then removed (together with other meta-keys) before kwargs go to ChatOllama.
    supports_format = kwargs.get("structured_output", True)
    if not supports_format and "format" in kwargs:
        model_name = kwargs.get("model", "?")
        log_key = f"{model_key}:{model_name}"
        if log_key not in _STRIP_FORMAT_LOGGED:
            _STRIP_FORMAT_LOGGED.add(log_key)
            print(
                f"[ollama] structured_output=false for '{model_key}' ({model_name}); "
                f"stripping format={kwargs['format']!r} — MLX runner does not support "
                f"the format parameter. Relying on prompt instructions + _extract_json."
            )
        kwargs.pop("format", None)

    # Strip project-level meta-keys that must not be forwarded to ChatOllama
